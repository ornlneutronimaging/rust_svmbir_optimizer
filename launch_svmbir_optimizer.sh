#!/usr/bin/env bash
# Launch the SVMBIR Optimizer GUI, rebuilding first if the sources changed.
#
# Usage: ./launch_svmbir_optimizer.sh [checkpoint.h5]
set -euo pipefail

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BINARY="$REPO_DIR/target/release/svmbir_optimizer"
...
if $needs_build; then
    ...
    echo "Building svmbir_optimizer (release)..."
